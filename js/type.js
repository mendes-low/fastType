...
function newGame() {
    ...
    // Генерируем сразу много слов
    for (let i = 0; i < 300; i++) {
        const rand = wordsList[Math.floor(Math.random() * wordsList.length)];
        wordsWrapper.innerHTML += formatWord(rand);
    }
    
    wordsWrapper.firstChild.classList.add("current");
    wordsWrapper.firstChild.firstChild.classList.add("current");
...
function handleShift(currentWord) {
    const wordHeight = 35; // из CSS
    // Если текущее слово перешло на вторую строку (70px от верха)
    if (currentWord.offsetTop > lineShift + wordHeight) {
        lineShift += wordHeight;
        wordsWrapper.style.marginTop = `-${lineShift}px`;
    }
}
...
inputField.addEventListener("keydown", (ev) => {
    ...
    if (key === " ") {
        ...
        handleShift(nextWord); // Проверка сдвига строки
        inputField.value = "";
    }
